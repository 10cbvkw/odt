...
class Lattice:
    def __init__(
        ...
        lambda_ = 532.8 * 1e-9
        RI_ = 1.33
        pixel_size = 5.86 * 1e-6 / 40
        pixel_size_k = 1 / (pixel_size * 256)
        scale = 1 / (pixel_size_k * 256) 
        k_m = 2 * np.pi / lambda_ * RI_
        k_x = x0.ravel() / scale
        k_y = x1.ravel() / scale
        k_z = np.sqrt(k_m**2 - k_x**2 - k_y**2)
        k_z_bar = (k_m - k_z) * scale
        print('lambda', lambda_)
        print('RI_', RI_)
        print('pixel_size', pixel_size)
        print('pixel_size_k', pixel_size_k)
        print('scale', scale)
        print('k_m', k_m)
        print('k_x', k_x[0])
        print('k_y', k_y[0])
        print('k_z', k_z[0])
        print('k_z_bar', k_z_bar[0])
        print(x0.ravel().shape, x1.ravel().shape, k_z_bar.shape)
        coords_odt = np.stack([x0.ravel(), x1.ravel(), k_z_bar], 1).astype(
            np.float32
        )
        self.coords_odt = torch.tensor(coords_odt, device=device)
        self.extent = extent
        self.D = D
        self.D2 = int(D / 2)
