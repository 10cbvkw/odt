...
import ctf, dataset, lie_tools, utils
from beta_schedule import LinearSchedule, get_beta_schedule
import config
from lattice import Lattice
from losses import EquivarianceLoss
from models import HetOnlyVAE, unparallelize
from pose_search import PoseSearch
...
def pretrain(model, lattice, optim, minibatch, tilt, zdim):
    ...
    def gen_slice(R):
        ...
        return _model.decode(lattice.coords_odt[mask] @ R, z).view(B, -1)
...
def train(
    ...
    def gen_slice(R):
        slice_ = model(lattice.coords_odt[mask] @ R, z).view(B, -1)
        if ctf_i is not None:
...
def save_config(args, dataset, lattice, model, out_config):
    ...
    config_.save(config, out_config)
